...
from shared_variables import activate_settings, path_to_declare_model_file, get_int_from_unicode
from formula_verificator import verify_with_data
from support_scripts.prepare_data_resource import amplify, get_symbol_ampl, select_declare_verified_traces, \
    ...
    with open('output_files/final_experiments/results/declare/%s' % eventlog, 'wb') as csvfile:
        ...
        for prefix_size in range(prefix_size_pred_from, prefix_size_pred_to):
            ...
            for line, line_id, line_group, times, times2, times3, times4 in izip(lines_s,
                ...
                compliant_predictions = PriorityQueue()
                non_compliant_predictions = PriorityQueue()
                ...
                for i in range(predict_size):
                    for k in range(current_beam_size):
                        ...
                        for j in range(current_beam_size):
                            ...
                            if temp_prediction == '!':
                                print '-------------------'
                                average_probability = current_prediction_premis.probability_of + \
                                    y_char_adjusted[indexes[0]] / \
                                    (len(current_prediction_premis.cropped_line[prefix_size:])+1)

                                print 'INFORMATION: ' + str(counterr) + ' ' + str(i) + ' ' + str(k) + ' ' + \
                                      str(j) + ' ' + current_prediction_premis.cropped_line[prefix_size:] + "     " + \
                                      ground_truth + "     " + \
                                      current_prediction_premis.cropped_line_group[prefix_size:] + "     " + \
                                      ground_truth_group + "     " + str(current_prediction_premis.probability_of)
                                ...
                                if verify_with_data(path_to_declare_model_file,
                                    ...
                                    queue_next_steps = PriorityQueue()
                                    compliant_predictions.put((average_probability, current_prediction_premis))
                                    print 'INFORMATION: ' + str(counterr) + ' ' + str(i) + ' ' + str(k) + ' ' + \
                                          str(j) + ' ' + current_prediction_premis.cropped_line[
                                                         prefix_size:] + "     " + \
                                          ground_truth + "     " + \
                                          current_prediction_premis.cropped_line_group[prefix_size:] + "     " + \
                                          ground_truth_group + "     " + str(current_prediction_premis.probability_of) \
                                          + "     " + str(average_probability)
                                    break
                                else:
                                    non_compliant_predictions.put((average_probability, current_prediction_premis))
                                    continue
                ...
                if compliant_predictions.empty():
                    _, best_prediction = non_compliant_predictions.get()
                    len_pred = len(best_prediction.cropped_line)
                    predicted = (best_prediction.cropped_line[prefix_size:len_pred - 1])
                    predicted_group = (best_prediction.cropped_line_group[prefix_size:len_pred - 1])
                    total_predicted_time = best_prediction.total_predicted_time
                else:
                    _, best_prediction = compliant_predictions.get()
                    len_pred = len(best_prediction.cropped_line)
                    predicted = (best_prediction.cropped_line[prefix_size:len_pred-1])
                    predicted_group = (best_prediction.cropped_line_group[prefix_size:len_pred-1])
                    total_predicted_time = best_prediction.total_predicted_time
