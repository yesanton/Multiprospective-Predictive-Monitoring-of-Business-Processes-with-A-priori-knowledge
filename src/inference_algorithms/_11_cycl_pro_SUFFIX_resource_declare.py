                        ...
                        for j in range(beam_size):
                            ...
                            if temp_prediction == '!':
                                if verify_with_data(path_to_declare_model_file,
                                    ...
                                    stop_symbol_probability_amplifier_current = 1
                                    lst = []
                                    for activity, group in izip(current_prediction_premis.cropped_line,
                                                                current_prediction_premis.cropped_line_group):
                                        lst.append((getInt_fromUnicode(activity), getInt_fromUnicode(group)))
                                    logging.info("Predicted -> %s: %s - verified (k=%s  j=%s)", line_id, lst, k, j)
                                    ...
                                    for activity, group in izip(current_prediction_premis.cropped_line,
                                                                current_prediction_premis.cropped_line_group):
                                        lst.append((getInt_fromUnicode(activity), getInt_fromUnicode(group)))
                                    logging.info("Predicted -> %s: %s - violated (k=%s  j=%s)", line_id, lst, k, j)
                                    violation_counter += 1
                                    continue
                            ...
                            temp_state_data = encode(temp_cropped_line,
                                                     ...
                                                     divisor,
                                                     divisor2)
                            probability_this = np.sort(y_char)[len(y_char) - 1 - j] + np.sort(y_group)[len(y_group) - 1 - j]
